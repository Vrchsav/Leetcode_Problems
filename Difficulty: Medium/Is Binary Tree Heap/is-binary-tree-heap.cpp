...
class Solution {
  private:
    ...
    bool iscomplete(struct Node*tree,int index,int cnt){
        if(tree==NULL){
            return true;
        }
        if(index>=cnt){
            ...
            return (iscomplete(tree->left,2*index+1, cnt)&& iscomplete(tree->right, 2*index+2, cnt));
        }
    }
    ...
    bool isHeap(struct Node* tree) {
        ...
        if(iscomplete(tree, index, countt) && ismax(tree)){
            return true;
        }
        else return false;
    }
